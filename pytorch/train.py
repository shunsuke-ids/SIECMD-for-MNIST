...
def set_seed(seed=42):
    """再現性のためのシード設定"""
    np.random.seed(seed)  # train_test_split等で使用
    torch.manual_seed(seed)  # モデル重み初期化、DataLoaderシャッフル
    torch.cuda.manual_seed_all(seed)  # GPU演算
    torch.backends.cudnn.deterministic = True  # CuDNN決定的動作
